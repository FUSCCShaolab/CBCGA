...
#只需改动
id=intersects(rownames(Trainset_IHC),rownames(Trainset_Clin))
Train=cbind(Trainset_Clin[match(id,rownames(Trainset_Clin)),],Trainset_IHC[match(id,rownames(Trainset_IHC)),])
colnames(Train)=c('ajcc','IHC')
rownames(Train)=id
Test=cbind(test_Clin,test_IHC)
colnames(Test)=c('ajcc','IHC')
rownames(Test)=rownames(test_Met)
#修改不合适列名 不需要修改
