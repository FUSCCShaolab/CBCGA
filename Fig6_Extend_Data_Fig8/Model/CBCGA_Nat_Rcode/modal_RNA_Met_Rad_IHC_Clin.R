...
#改这一块就可以
id=intersects(rownames(Trainset_IHC),rownames(Trainset_Clin),rownames(Trainset_Met),rownames(Trainset_RNA),rownames(Trainset_rad))
Train=cbind(Trainset_rad[match(id,rownames(Trainset_rad)),],Trainset_RNA[match(id,rownames(Trainset_RNA)),],Trainset_Met[match(id,rownames(Trainset_Met)),],Trainset_Clin[match(id,rownames(Trainset_Clin)),],Trainset_IHC[match(id,rownames(Trainset_IHC)),])
Test=cbind(test_rad,test_rna,test_Met,test_Clin,test_IHC)
#修改不合适列名 不需要修改
