...
#只需改动
id=intersects(rownames(Trainset_Met))
Train=cbind(Trainset_Met[match(id,rownames(Trainset_Met)),])
Test=cbind(test_Met)
...
#yTrain=cbind(Trainrfs,Trainrfstime)
#colnames(yTrain)=c('Status','Survival_in_days')
#yTest=cbind(Testrfs,Testrfstime)
#colnames(yTest)=c('Status','Survival_in_days')